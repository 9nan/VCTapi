...
def get_match_details(match_id):
    ...
    This function gets the data from the live matches endpoint to ensure consistency
    
    Args:
        match_id (str): The VLR match ID (e.g., '551847' from the match URL)
    """
    try:
        # Get live matches data
        live_matches = vlr_live_score()
        
        # Check if we got valid data
        if not live_matches or 'data' not in live_matches or 'segments' not in live_matches['data']:
            return {"status": "error", "message": "Could not fetch live matches data"}
        
        # Find the match with the specified ID
        target_match = None
        for match in live_matches['data']['segments']:
            if match_id in match.get('match_page', ''):
                target_match = match
                break
        
        if not target_match:
            return {"status": "error", "message": f"Match with ID {match_id} not found in live matches"}
        
        # Return the match data in the expected format
        return {
            "status": "success",
            "data": {
                "team1": target_match.get('team1', ''),
                "team2": target_match.get('team2', ''),
                "flag1": target_match.get('flag1', ''),
                "flag2": target_match.get('flag2', ''),
                "team1_logo": target_match.get('team1_logo', ''),
                "team2_logo": target_match.get('team2_logo', ''),
                "score1": target_match.get('score1', '0'),
                "score2": target_match.get('score2', '0'),
                "team1_round_ct": target_match.get('team1_round_ct', 'N/A'),
                "team1_round_t": target_match.get('team1_round_t', 'N/A'),
                "team2_round_ct": target_match.get('team2_round_ct', 'N/A'),
                "team2_round_t": target_match.get('team2_round_t', 'N/A'),
                "map_number": target_match.get('map_number', ''),
                "current_map": target_match.get('current_map', ''),
                "time_until_match": target_match.get('time_until_match', 'LIVE'),
                "match_event": target_match.get('match_event', ''),
                "match_series": target_match.get('match_series', ''),
                "unix_timestamp": target_match.get('unix_timestamp', ''),
                "match_page": target_match.get('match_page', f'https://www.vlr.gg/{match_id}')
            }
        }
